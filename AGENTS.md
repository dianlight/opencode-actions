...
`data/model-config.json` is generated by the maintenance script (per workflow stem + job id: `go`/`free` model pairs, using the audit recommendations). The maintenance workflow commits it to `main`. Every OpenCode step resolves its model via `resolve-model.sh` (`model: ${{ steps.resolve.outputs.MODEL }}`), so model upgrades land without editing workflow files — the audit marks these steps with ⚙️.

To change a model, never edit workflow files: regenerate the config by running `mise run maintenance` (writes `data/model-config.json`) and commit it — downstream workflows pick it up automatically at their next run.
